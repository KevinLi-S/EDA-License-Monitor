    ...
    def parse(self, raw_text: str) -> ServerSnapshot:
        ...
        while i < len(lines):
            ...
            while i < len(lines):
                ...
                if checkout_match and checkout_match.group('user') and 'license' not in next_line.strip().lower():
                    display = checkout_match.group('display')
                    client = checkout_match.group('client')
                    process_info = next_line.strip()
                    if client and client != display:
                        process_info = f"{client} | {process_info}"
                    checkouts.append(
                        CheckoutRecord(
                            username=checkout_match.group('user'),
                            hostname=checkout_match.group('host'),
                            display=display,
                            server_handle=checkout_match.group('handle') or checkout_match.group('version'),
